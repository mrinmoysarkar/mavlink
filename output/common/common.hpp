...
//! MAV_TYPE ENUM_END
constexpr auto MAV_TYPE_ENUM_END = 32;
...
enum class MAV_CMD : uint16_t
{
    ...
    IMAGE_START_CAPTURE=2000, /* Start image capture sequence. Sends CAMERA_IMAGE_CAPTURED after each capture. Use NAN for reserved values. |Reserved (Set to 0)| Duration between two consecutive pictures (in seconds)| Number of images to capture total - 0 for unlimited capture| Reserved (all remaining params)|  */
...
enum class MAV_BATTERY_CHARGE_STATE : uint8_t
{
    ...
    OK=1, /* Battery is not nearly empty, normal operation | */
    LOW=2, /* Battery state is low, warn and monitor close | */
    CRITICAL=3, /* Battry state is critical, return / abort immediately | */
    EMERGENCY=4, /* Battry state is too low for ordinary abortion, fastest possible emergency stop preventing damage | */
    FAILED=5, /* Battry failed, damage unavoidable | */
    UNHEALTHY=6, /* Battry is diagnosed to be broken or an error occurred, usage is discouraged / prohibited | */
};
...
#include "./mavlink_msg_custom_msg.hpp"
